...
function formInit() {
	...
	function formSubmit() {
		...
		function formSent(form, responseResult = ``) {
			...
			// Удаляем класс active у кнопок очистки в форме
			form.querySelectorAll('[data-fls-clear]').forEach(btn => btn.classList.remove('active'));

			// Добавляем класс --sent
			form.classList.add('--sent');

			// Удаляем класс --sent через 5 секунд
			setTimeout(() => {
	...
	function formFieldsInit() {
		...
		document.body.addEventListener("focusout", function (e) {
			...
			if (targetElement.tagName === 'INPUT' || targetElement.tagName === 'TEXTAREA') {
				...
				// Если фокус уходит на кнопку очистки внутри той же обёртки — не убираем фокус
	...
	// ОЧИСТКА ПОЛЯ ИНПУТ ПРИ НАЛИЧИИ КНОПКИ ОЧИСТКИ
	function formClearButtonInit() {
		...
		document.body.addEventListener('click', function (e) {
			...
			if (input) {
				input.value = '';
				clearBtn.classList.remove('active');
				input.focus(); // можно удалить, если не нужно автофокусить
			}
		});
	}
